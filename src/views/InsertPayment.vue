<template>
    <v-container fluid class="fill-height">
        <v-row
            ...
            justify="center"
        >
            <v-col cols="12" v-if="saldo.type == 1">
                <v-alert
                    dismissible
                    color="cyan"
                    border="left"
                    ...
                    >
                    Tienes un saldo a favor de <strong>${{saldo.data}}</strong> ( sera repatido automaticamente en el siguiente pago).
                </v-alert>
            </v-col>
            <v-col cols="12" sm="12" md="12" class="text-center" v-if="dataSendApi">
                <v-alert type="info">
                    Este es tu codigo de reconocimiento de pago, 
                    <b class="">haz un foto con tu telefono</b>,
                    cualquier consulta o error dar este codigo : 
                    <b class="third--text">{{dataSendApi.code}}</b>
                ...
                <v-sheet
                  ...
                  >
                    ...
                    <h1 class="black--text pt-10 pb-3">
                      ...
                      {{total+saldo.data}} / {{dataSendApi.total}}
                    </h1>
                    </div>
                </v-sheet>
            </v-col>
            ...
            <div style="display:none">{{finalPayment}}</div>
        </v-row>
    </v-container>
</template>
<script>
...
export default {
    ...
    computed:{
        dataSendApi(){
            console.log('detalle',this.$store.getters.dataSendApi)
            // this.increment()
            this.totalApagar = this.$store.getters.dataSendApi.total
            this.items = this.$store.getters.dataSendApi.items
            if(this.$store.getters.dataSendApi.saldo){
                this.saldo.type = this.$store.getters.dataSendApi.saldo.saldo < 0 ? 1 : 0 
                this.saldo.data = this.$store.getters.dataSendApi.saldo.saldo < 0 ? (this.$store.getters.dataSendApi.saldo.saldo*-1) : 0
                console.log('saldo',this.saldo)
                // this.total = 1000
            }
            
            this.paymentId = this.$store.getters.dataSendApi.paymentId
        ...
        finalPayment(){
            console.log('finalPayment',this.$store.getters.finalPayment)
            if(this.$store.getters.finalPayment.status){
                this.$router.push({ name: 'FinishPayment'})
            }
    ...
    methods:{
            ...
            this.interval = setInterval(() => {
                Api.verifyIncome().then(response=>{
                    ...
                    if(that.total != valor){
                        ...
                        Api.addTickets({paymentId:that.paymentId,code:code}).then(response=>{
        ...
        finishPayment(){
            if((this.total + this.saldo.data) == 0){
        ...
        finishPaymentMachine(){
            Api.finishPayment().then(response=>{
                ...
                this.totalPayment.tt = this.total + this.saldo.data
                this.totalPayment.saldo = this.saldo.data
                ...
                this.$store.dispatch('finalPayment',{data:this.totalPayment,status:true})
